...
class ExhaustiveSearch(SearchSpace):
    def __init__(self, search_space=[[-3.0, 3.0], [-3.0, 3.0], [-10.0, 10.0]], search_dims=[7, 7, 3]):

        x_space = np.linspace(search_space[0][0], search_space[0][1], search_dims[0])

        y_space = np.linspace(search_space[1][0], search_space[1][1], search_dims[1])

        a_space = np.linspace(search_space[2][0], search_space[2][1], search_dims[2])
...
class ParticleFiltering(SearchSpace):
    ...
    def sample(self, n=10):
        ...
        for i in range(n):
            if len(self.data) > 0:
                i_mean = np.random.choice(list(range(len(self.data))), p=self.score / np.linalg.norm(self.score, ord=1))
                sample = np.random.multivariate_normal(mean=self.data[i_mean], cov=np.diag(np.array(self.bnd)))
            else:
                sample = np.random.multivariate_normal(
                    mean=np.zeros(len(self.bnd)), cov=np.diag(np.array(self.bnd) * 3)
                )
...
class GaussianMixtureModel(SearchSpace):
    ...
    def addData(self, data, score):
        ...
        try:
            model = GeneralMixtureModel.from_samples(
                MultivariateGaussianDistribution, n_components=self.n_comp, X=self.data, weights=score_normed
            )
            self.model = model
        except:
    ...
    def reset(self, n_comp=5):
        ...
        self.score = self.score / np.linalg.norm(self.score, ord=1)
        if self.dim == 2:
            self.model = MultivariateGaussianDistribution(np.zeros(self.dim), np.diag([1.0, 1.0]))
        else:
            self.model = MultivariateGaussianDistribution(np.zeros(self.dim), np.diag([1.0, 1.0, 5.0]))
