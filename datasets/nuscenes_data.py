...
class NuScenesDataset(base_dataset.BaseDataset):
    ...
    def filter_instance(self, split, category_name=None, min_points=-1):
        ...
        for instance in self.nusc.instance:
            ...
            if (
                scene["name"] in scene_splits[split]
                and anno["num_lidar_pts"] >= min_points
                and (category_name is None or category_name is not None and instance_category in general_classes)
            ):
    ...
    def _get_frame_from_anno_data(self, anno):
        ...
        bb = Box(
            box_anno["translation"],
            box_anno["size"],
            Quaternion(box_anno["rotation"]),
            name=box_anno["category_name"],
            token=box_anno["token"],
        )
