...
class WaymoDataset(base_dataset.BaseDataset):
    def __init__(self, path, split, category_name="VEHICLE", **kwargs):
        ...
        if self.tiny:
            length = len(self.tracklet_anno_list)
            self.tracklet_anno_list = self.tracklet_anno_list[: length // 10]
            self.tracklet_len_list = self.tracklet_len_list[: length // 10]
    ...
    def _load_data(self):
        ...
        if self.tiny:
            preload_data_path = os.path.join(
                self.Waymo_Folder, f"preload_{self.split}_{self.category_name}_{self.preload_offset}_tiny.dat"
            )
        else:
            preload_data_path = os.path.join(
                self.Waymo_Folder, f"preload_{self.split}_{self.category_name}_{self.preload_offset}.dat"
            )
        ...
        frame_num = sum(list(map(lambda x: len(x), training_samples)))
        print(frame_num)
    ...
    def _build_tracklet_anno(self):
        preload_data_path = os.path.join(
            self.Waymo_Folder, f"sot_infos_{self.category_name.lower()}_{self.split.lower()}.pkl"
        )
    ...
    def _get_frame_from_anno(self, anno, track_id=None, check=False):
        ...
        """
        sample_data_lidar = os.path.join(self.Waymo_Folder, anno["PC"])
        gt_boxes = anno["Box"]
        ...
        pointcloud[:3, :] = global_from_car.dot(np.vstack((pointcloud[:3, :], np.ones(nbr_points))))[:3, :]
        ...
        bb = Box(
            gt_boxes[0:3],
            gt_boxes[3:6],
            Quaternion(axis=[0, 0, 1], radians=-gt_boxes[-1]),
            velocity=gt_boxes[6:9],
            name=anno["Class"],
        )
        ...
        if check:
            from datasets.utils import box2obj, get_3d_box, write_bbox, write_obj
