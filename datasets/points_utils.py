...
def random_choice(num_samples, size, replacement=False, seed=None):
    ...
    return torch.multinomial(
        torch.ones((size), dtype=torch.float32), num_samples=num_samples, replacement=replacement, generator=generator
    )
...
def getOffsetBB(box, offset, degrees=True, use_z=False, limit_box=True):
    ...
    if degrees:
        if len(offset) == 3:
            new_box.rotate(Quaternion(axis=[0, 0, 1], degrees=offset[2]))
        elif len(offset) == 4:
            new_box.rotate(Quaternion(axis=[0, 0, 1], degrees=offset[3]))
    else:
        if len(offset) == 3:
            new_box.rotate(Quaternion(axis=[0, 0, 1], radians=offset[2]))
        elif len(offset) == 4:
            new_box.rotate(Quaternion(axis=[0, 0, 1], radians=offset[3]))
    if limit_box:
