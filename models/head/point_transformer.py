...
class PointTransformerLayer(nn.Module):
    def __init__(
        self, *, dim, pos_mlp_hidden_dim=64, attn_mlp_hidden_mult=4, num_neighbors=None, num_pos=3, scalar=False
    ):
        super().__init__()
        ...
        self.pos_mlp = nn.Sequential(
            nn.Linear(num_pos, pos_mlp_hidden_dim), nn.ReLU(), nn.Linear(pos_mlp_hidden_dim, dim),
        )

        if not scalar:
            self.attn_mlp = nn.Sequential(
                nn.Linear(dim, dim * attn_mlp_hidden_mult), nn.ReLU(), nn.Linear(dim * attn_mlp_hidden_mult, dim),
            )
        self.scalar = scalar
    ...
    def forward(self, x, pos, mask=None):
        ...
        if not self.scalar:
            ...
            qk_rel = torch.bmm(q, k.transpose(1, 2))
            # sim = qk_rel / sqrt(x.shape[-1])
            sim = qk_rel
