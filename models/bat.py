...
class BAT(base_model.BaseModel):
    def __init__(self, config=None, **kwargs):
        ...
        self.rpn = P2BVoteNetRPN(
            self.config.feature_channel,
    ...
    def forward(self, input_dict):
        ...
        # box-aware xcorr
        fusion_feature = self.xcorr(
            template_feature, search_feature, template_xyz, search_xyz, template_bc, pred_search_bc
        )
        # proposal generation
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_fps = self.batch_size / (current_time - prev_time)
        self.log("fps", batch_fps, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        ...
        self.log(
            "loss_box/train", loss_dict["loss_box"].item(), on_step=True, on_epoch=True, prog_bar=True, logger=False
        )
        self.log(
            "loss_seg/train", loss_dict["loss_seg"].item(), on_step=True, on_epoch=True, prog_bar=True, logger=False
        )
        self.log(
            "loss_vote/train", loss_dict["loss_vote"].item(), on_step=True, on_epoch=True, prog_bar=True, logger=False
        )
        self.log("loss_bc/train", loss_dict["loss_bc"].item(), on_step=True, on_epoch=True, prog_bar=True, logger=False)
        self.log(
            "loss_objective/train",
            loss_dict["loss_objective"].item(),
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=False,
