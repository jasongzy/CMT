...
def estimateAccuracy(box_a, box_b, dim=3, up_axis=(0, -1, 0)):
    if dim == 3:
        ...
        return np.linalg.norm(box_a.center[up_axis != 0] - box_b.center[up_axis != 0], ord=2)
...
class TorchPrecision(Metric):
    ...
    def value(self, accs):
        prec = [torch.sum((accs <= thres).float()) / len(accs) for thres in self.Xaxis]
...
class TorchSuccess(Metric):
    ...
    def value(self, overlaps):
        succ = [torch.sum((overlaps >= thres).float()) / len(overlaps) for thres in self.Xaxis]
